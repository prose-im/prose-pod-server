...
/// Tests that the restore atomicity logic doesn’t fail if a `*.bak` file
/// already exists.
#[tokio::test(flavor = "multi_thread")]
async fn error_path_restore_atomicity_unique_bak() -> Result<(), anyhow::Error> {
    let context = init();
    let TestContext {
        now,
        ref test_data_path,
    ...
    println!();
    assert!(test_data_path.join("foo.bak").is_dir());
    service
        .restore_backup(&backup_id, &blueprint)
        .await
        .unwrap();
    // Ensure `foo.bak` is not deleted.
    assert!(test_data_path.join("foo.bak").is_dir());
